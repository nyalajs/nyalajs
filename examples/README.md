...
| [docs-site](./docs-site) | [`inertia-starter`](../templates/inertia-starter) | Full CRUD docs platform — real doc content in SQLite (seeded from Nyala's actual `website/docs/*.md`), with create/edit/delete forms, real Shiki-highlighted markdown rendering, and a Laravel-docs-style UI (search, on-this-page outline, prev/next nav). |
...
Swap `todo-api` for `helpdesk-saas`, `devblog-cms`, or `docs-site` — same flow, each README documents its specific env vars and seeded data.
